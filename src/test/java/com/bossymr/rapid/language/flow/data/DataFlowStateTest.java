...
class DataFlowStateTest {
    ...
    void getConstraintFromCondition() {
        ...
        snapshot = new VariableSnapshot(RapidPrimitiveType.NUMBER);
        state.assign(new Condition(snapshot, ConditionType.EQUALITY, new BinaryExpression(BinaryOperator.MULTIPLY, ConstantValue.of(RapidPrimitiveType.NUMBER, 5), ConstantValue.of(RapidPrimitiveType.NUMBER, 10))), true);
    ...
    void getConstraintWithRelation() {
        DataFlowState state = DataFlowState.createState(getEmptyFunctionBlock());
        VariableSnapshot reference = new VariableSnapshot(RapidPrimitiveType.NUMBER);
        ...
        VariableSnapshot snapshot = new VariableSnapshot(RapidPrimitiveType.NUMBER);
        state.assign(new Condition(snapshot, ConditionType.EQUALITY, new BinaryExpression(BinaryOperator.MULTIPLY, ConstantValue.of(RapidPrimitiveType.NUMBER, 5), reference)), true);
