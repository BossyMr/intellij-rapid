...
public class RapidParser implements PsiParser, LightPsiParser {
  ...
  public static boolean conditional_argument(PsiBuilder b, int l) {
      ...
      r = r && conditional_argument_3(b, l + 1);
      exit_section_(b, m, CONDITIONAL_ARGUMENT, r);
      return r;
  }

  // '<VAR>' | empty_expression
  private static boolean conditional_argument_3(PsiBuilder b, int l) {
      if (!recursion_guard_(b, l, "conditional_argument_3")) return false;
      boolean r;
  ...
  static boolean next_argument(PsiBuilder b, int l) {
      ...
      boolean r;
      Marker m = enter_section_(b);
      r = next_argument_0(b, l + 1);
      if (!r) r = next_argument_1(b, l + 1);
      exit_section_(b, m, null, r);
      return r;
  }

  // conditional_argument | optional_argument
  private static boolean next_argument_0(PsiBuilder b, int l) {
      if (!recursion_guard_(b, l, "next_argument_0")) return false;
      boolean r;
  ...
  // ',' ('<ARG>' | conditional_argument | optional_argument | required_argument)
  private static boolean next_argument_1(PsiBuilder b, int l) {
      if (!recursion_guard_(b, l, "next_argument_1")) return false;
      boolean r;
      Marker m = enter_section_(b);
      r = consumeToken(b, COMMA);
      r = r && next_argument_1_1(b, l + 1);
      exit_section_(b, m, null, r);
      return r;
  }

  // '<ARG>' | conditional_argument | optional_argument | required_argument
  private static boolean next_argument_1_1(PsiBuilder b, int l) {
      if (!recursion_guard_(b, l, "next_argument_1_1")) return false;
      boolean r;
      r = consumeToken(b, ARG_PLACEHOLDER);
      if (!r) r = conditional_argument(b, l + 1);
      if (!r) r = optional_argument(b, l + 1);
  ...
  public static boolean optional_argument(PsiBuilder b, int l) {
      ...
      r = consumeToken(b, BACKSLASH);
      r = r && unqualified_reference_expression(b, l + 1);
      r = r && optional_argument_2(b, l + 1);
      exit_section_(b, m, OPTIONAL_ARGUMENT, r);
      return r;
  }

  // [':=' empty_expression]
  private static boolean optional_argument_2(PsiBuilder b, int l) {
      if (!recursion_guard_(b, l, "optional_argument_2")) return false;
      optional_argument_2_0(b, l + 1);
    return true;
  }

  // ':=' empty_expression
  private static boolean optional_argument_2_0(PsiBuilder b, int l) {
      if (!recursion_guard_(b, l, "optional_argument_2_0")) return false;
      boolean r;
      Marker m = enter_section_(b);
      r = consumeToken(b, CEQ);
      r = r && empty_expression(b, l + 1, -1);
  ...
  private static boolean required_argument_0(PsiBuilder b, int l) {
      ...
      required_argument_0_0(b, l + 1);
    return true;
  }

  // unqualified_reference_expression ':='
  private static boolean required_argument_0_0(PsiBuilder b, int l) {
      if (!recursion_guard_(b, l, "required_argument_0_0")) return false;
      boolean r;
      Marker m = enter_section_(b);
      r = unqualified_reference_expression(b, l + 1);
      r = r && consumeToken(b, CEQ);
